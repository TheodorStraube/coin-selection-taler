...
void simulate_user_actions(int user_index, User user, Wallet denomination_wallet, int num_actions, strategy strategy) {
    ...
    const char* StrategyNames[] = {"MAX_BILLS", "MIN_BILLS", "CLOSEST_TO_EXPIRE_MIN_BILLS", "CLOSEST_TO_EXPIRE_MAX_BILLS", "MAX_BILLS_TIME_TO_EXPIRE_WEIGHTED", "RANDOM", "EVEN_FROM_MIN_TO_MAX", "EVEN_FROM_MAX_TO_MIN", "GREEDY_MIN_TO_MAX","GREEDY_MIN_TO_MAX_FIX", "CALL_EXTERNAL", "WALLET_CORE"};
        ...
        for (int i = 0; i < num_actions; i++) {
            ...
            if (user.actions[i].operation == WITHDRAW_OP || user.actions[i].operation == REFUND_OP) {
                ...
                if (!allocatedCoins) {
                    ...
                    if(!covers_amount(allocatedCoins, allocatedCoinCount, transaction_amount)){
                        char error[1024];
                        sprintf(error, "Allocated coins incorrect. %d coins for denomination %d. [%s]", user.wallet.num_coins, transaction_amount, StrategyNames[strategy]);
                        printf("%s\n", error);
                    }else {
