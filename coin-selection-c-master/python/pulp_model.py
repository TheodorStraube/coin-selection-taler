...
def select_minimize_fee(amount, wallet_data):
    ...
    prob += (
        lpSum(
            [
                wallet.coins[i].denomination.amount * selection[i]
                for i, coin in enumerate(wallet.coins)
            ]
        )
        >= amount,
...
def select_minimize_deposit_refresh(amount, wallet_data, refresh_weight=0.5):
    ...
    #    print(coin.denomination.amount, f"{coin.denomination.rules.durations.legal}/{SECONDS_IN_3_YEARS} {1 - coin.denomination.rules.durations.legal/SECONDS_IN_3_YEARS}")
...
def select_minimize_wallet_refresh(amount, wallet_data, refresh_weight=0.5):
    wallet = parse_wallet(wallet_data)
    ...
    prob += (
        lpSum(
            [
                coin.denomination.rules.fees.deposit_fee * selection[i]
                + coin.denomination.rules.fees.refresh_fee
                * (SECONDS_IN_3_YEARS - coin.denomination.rules.durations.legal)
                * (1 - selection[i])
                for i, coin in enumerate(wallet.coins)
            ]
        ),
